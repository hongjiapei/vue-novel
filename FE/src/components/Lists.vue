<template>
  <div>
    <mu-appbar style="width: 100%;position: fixed;top: 0;" :title="category_name" :color="'rgb(236, 81, 134)'">
      <span slot="left">
        <mu-icon value="navigate_before" @click="goBack"></mu-icon>
      </span>
    </mu-appbar>
    <div style="margin-top: 60px;" v-if="lists.length > 0">
      <mu-list>
      ...
      <mu-row gutter>
        <mu-col span="3" @click="changePage(first_url)"><mu-button :disabled="category_url === first_url" :color="'rgb(233, 30, 99)'">首页</mu-button></mu-col>
        <mu-col span="3" @click="changePage(prev_url)"><mu-button :disabled="category_url === prev_url" :color="'rgb(233, 30, 99)'">上页</mu-button></mu-col>
        <mu-col span="3" @click="changePage(next_url)"><mu-button :disabled="category_url === next_url" :color="'rgb(233, 30, 99)'">下页</mu-button></mu-col>
        <mu-col span="3" @click="changePage(last_url)"><mu-button :disabled="category_url === last_url" :color="'rgb(233, 30, 99)'">尾页</mu-button></mu-col>
      </mu-row>
    </div>
  </div>
</template>
